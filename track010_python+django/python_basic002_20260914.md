...
```bash
#pip : Node.js 의 npm, Java Gradel/Maven 같은 패키지 설치 관리자
#최신라이브러이 설치, 호환성 에러 방지 업그레이드
...
# Template - 화면구성 : html, jsp, thymelaef,,,
...
# 답: 1) ( 비즈니스로직처리  ) / 2) ( DB와 Template 연결  )
...
```python
# 수정 파일 경로 - analysis_pjt - settings.py
...
# @admin.register(ServiceLog) admin
...
```bash
# 1. 마이그레이션 생성 - models.py 변경사항을 db설계도를 생성
python manage.py makemigrations

# 2. DB 적용 - db.sqllite3 데이터베이스에 반영해서 테이블을 생성
...
## M(model) T(view) V(Controller: 비지니스로직, db와 view와연결)
