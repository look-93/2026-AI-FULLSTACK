...

{
  ...
  "scripts": {
    ...
    "test": "jest" // jest 활용해서 테스트코드 실행 -> junit과 같은
  },
  "author": "sally",
  "license": "ISC",
  "dependencies": {
    ...
    "antd": "^4.8.6",
    "axios": "^1.3.4", // 백엔트 api 서버와 통신 http 라이브러리
    "babel-plugin-styled-components": "^1.12.0", // ssr 환경 클래스명 충돌방지
    "bootstrap": "^5.3.8", // 전반적인 웹 스타일링
    "cross-env": "^7.0.2", // 운영체제 상관없이 (window, mac, linux) 환경변수 설정
    "dotenv": "^16.0.1", // 중요키
    "express": "^5.2.1", // 웹서버 
    "faker": "^4.1.0", // 가짜데이터 무작위생성도구
    "immer": "^9.0.19", // 코드짧게 줄이기
    "jest": "^29.7.0", // 테스트도구
    "moment": "^2.29.1", // 날짜 및 시간데이터 포맥팅
    "next": "^12.3.4", // react 에서 ssr 지원 프레임워크
    "next-redux-wrapper": "^6.0.2", // 연결
    "pm2": "^4.5.0", // 배포환경에서 node.js 프로세스 계속 실행 되게 관리
    "prop-types": "^15.7.2", // 전달되는 props 타입 검사도구
    "react": "^17.0.2", 
  ...
  "jest": { // 테스트환경
    "testEnvironment": "jsdom", // 테스트가상환경 dom
    "setupFilesAfterEnv": [
...
2. 설정파일
front/
├── .babelrc      // Babel 설정파일: jsx -> 일반 js로 변환 브라우저 이해, 최신문법(async/await, optional, chaining 등)
├── .eslintrc     // ESLint 설정파일: 일관된 코드 스타일 유지(들여쓰기, 세미콜론, 따옴) / 팀규칙
└── setupTests.js // 테스트 환경 설정파일: 테스트환경 설정파일 환경초기
