...
1. 글자색상을 초록색으로 지정하는 속성은? color:green
1. 글자 크기를 30px로 지정하는 속성은? font-size:30px
1. 글자를 오른쪽으로 정렬하는 속성은? text-align:right
1. 글자에 취소선을 추가하는 속성은? text-decoration:line-through
1. 글꼴을 'Times New Roman'으로 지정하는 속성은? font-family:'Times New Roman'
1. 글자를 굵은표시를 기본으로 표시하는 속성은? font-weight:normal
1. 요소의 세로 길이를 400px로 지정하는 속성은? height:400px
1. 요소의 바깥쪽 여백을 20px로 지정하는 속성은? margin:20px
1. 요소의 안쪽 여백을 5px로 지정하는 속성은? padding:5px
1. 요소에 2px 점선 테두리를 추가하는 속성은? border:2px dotted red
1. 모서리를 둥글게 50%로 만드는 속성은? border-radius:50%
1. 그림자 효과를 빨간색으로 추가하는 속성은? box-shadow:0 0 10px rgba(0,0,0,0.5)
1. 빠르게 움직이는 장면전환효과를 주는 속성은? transition:all 0.2s;
...
5. 총점 구하기
6. 평균 구하기
7. 평균이 60점이상이고 각과목이 40점 미만이면 아니라면 합격/ 아니면 불합격
8. 평균이 95점이상이면 장학생
9. 평균이 90점이상이면 수, 80점이상이면 우, 70점이상이면 미, 60점이상이면 양, 아니라면 가
