...
public class ApiEmail {
	...
	@Value("${naver.user}") private String user;			// 이메일
	...
	public void sendEmail(String subject, String content, String to) {
	      ...
	      props.put("mail.smtp.auth", "true"); // "mail.smtp.auth" 쓸거니? "true"
	      ...
	      Session session = Session.getInstance(props, new Authenticator() {  // session : 로그인이 유지되있는 동안 아이디 비밀번호로 메일보낼게
