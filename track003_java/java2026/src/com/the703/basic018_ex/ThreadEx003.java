...
class Count extends Thread{
	...
	public void run() {
		for (int i = 10; i > 0; i--) {
			...
			//중단요청확인
			if(Thread.currentThread().isInterrupted()) { // 아래 걸려있어서 없어도됨
				break;
			}
			...
			try {
				Thread.sleep(1000); // 1초마다 해야하니까 sleep 
			} catch (InterruptedException e) { // interrupt 받았으면 중단
				break;
			}
		}
	}
...
public class ThreadEx003 {
	...
	public static void main(String[] args) {
		...
		String info = "계속 count";
		count.start();
		
		String answer = JOptionPane.showInputDialog("conunt stop? y/n");
		if(answer.equals("y")) {
			count.interrupt(); // y면 interrupt
			info = "count 중단";
		}
