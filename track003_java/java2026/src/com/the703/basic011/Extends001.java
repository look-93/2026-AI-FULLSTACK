...
//3. 상속 - 클래스의 재사용
/*  하위클래스에서 super() : 상위클래스 를 계속 호출
    제일 최상위 클래스의 }가 닫히면서 하위클래스들의 } 가 닫히면서 초기화 해줌
	Object #4  {	}#5
	  ↑
	  A    #3. {a=10}#6
	  ↑
	  ...
	  4) 객체 생성 순서 : Object → A → B → C 			5 6 7 8
*/
...
6. 문법 class 자식클래스 extends 부모클래스 (o) 
	   class 자식클래스 extends 부모클래스1,부모클래스2 (x) 
7. 상속시 부모속성을 사용할 수 있는 이유는? - 부모생성자를 호출해서 부모의 인스턴스변수를 초기화해 사용가능하게 만들어줌 
8. 오버라이드 - @Override - 상속시 부모의 메서드와 동일, 자식클래스에 맞게 수정해서 사용
 */