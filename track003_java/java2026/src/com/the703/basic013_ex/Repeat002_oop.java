...
public class Repeat002_oop {
    public static void main(String[] args) {
    	/*	method(정보, static, final): Account.class / Account.accountCount=2
			...
			[heap]							[stack]
		1번지: Account{balance=200,id=2} < 	a1(2번지)
...
- 캡슐화 - 외부에서 접근 제한
- private int balance;
- 값을 넣고 뺄때는 getter/setter를 사용한다.
- 메서드 파라미터를 통해서도 접근가능

- 정의 : 외부에서 접근 제한
- 구현 : balance를 private 선언, deposit, withdraw, getBalance메서드를 통해서만 접근
...
- static 으로 만들어진 클래스함수와 클래스인스턴스는 method area에 만들어진다.
- accountCount 
...
- final이 붙은 class나 멤버함수 멤버변수는 변경x,
- id - 초기화 후 사용 public Account(){this.id = ++accountCount;}

- 의미 : 변경불가 - 클래스(상속x), 변수(상수:수정안됨), 메서드(오버라이드x) 
- id, 한번 초기화되면 변경불가, this.id = ++accountCount; 초기화

Q5. Account.accountCount의 값은 얼마인가? 왜 그렇게 되는지 설명하시오.
- 2
- accountCount - static -객체생성시 method area에 생성됨
  a1.deposit(50); / a2.withdraw(30); 
  두번 호출되면서 두 번 카운트됨  
