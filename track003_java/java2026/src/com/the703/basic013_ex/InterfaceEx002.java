...
class Driver {	
	void drive(Vehicle ve) { // Vehicle ve = car, Vehicle ve = mo
		//만약에 MotorCycle 이라면 helmet()호출		
		if(ve instanceof MotorCycle) {
			((MotorCycle) ve).helmet();
		}
		
		ve.run(); // 기본다형성동작 - override 된 자식메서드
	}
}
