...
public class InterfaceEx003 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			// 부모 = 자식(Dog, Cat, Bird)
			// 업캐스팅
			// 부모타입으로 메서드호출시 오버라이드된 최신 자식 메서드가 호출
			//animal{sound} = [1번지] {@sound(), playFetch()} - {-오버라이드-}
			//animal{sound} = [2번지] {@sound(), scratch()}   - {-오버라이드-}
			//animal{sound} = [3번지] {@sound(), fly()} 	     - {-오버라이드-}
