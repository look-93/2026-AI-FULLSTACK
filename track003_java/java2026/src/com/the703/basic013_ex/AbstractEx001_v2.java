...
class CleaningRobot2 extends Robot2 {
	@Override
	void charge() {
		System.out.println(super.model + "청소로봇 충전 중... 배터리" + super.battery + "%");
	}

	@Override
	void move() {
		System.out.println(super.model + "청소로봇 : 청소 중");
	}

	@Override
	void speak() {
		System.out.println(super.model + "청소로봇: 먼지를 제거합니다!");
	}
}

class SecurityRobot2 extends Robot2 {
	@Override
	void charge() {
		System.out.println(super.model + "경비로봇 태양광 충전 중... 배터리" + super.battery + "%");
	}

	@Override
	void move() {
		System.out.println(super.model + "경비로봇 : 경비 중");
	}

	@Override
	void speak() {
		System.out.println(super.model + "경비로봇: 이상 없음. 안전 확보!");
	}
}

class CookingRobot2 extends Robot2 {
	@Override
	void charge() {
		System.out.println(super.model + "요리로봇 인덕션 충전 중... 배터리" + super.battery + "%");
	}

	@Override
	void move() {
		System.out.println(super.model + "요리로봇 : 요리 중");
	}

	@Override
	void speak() {
		System.out.println(super.model + "요리로봇: 오늘의 메뉴는 파스타입니다!");
	}
}


public class AbstractEx001_v2 {

	public static void main(String[] args) {
        //Robot robot  = new Robot();  // Q1.why? 오류이유?  abstract 클래스  
        System.out.println("\n--- 로봇 배열 시뮬레이션 ---");
        //  부모            자식
        Robot2 [] bots = { new CleaningRobot2(), new SecurityRobot2() , new CookingRobot2() };
        int[] levels = { 50, 70, 95 }; 
