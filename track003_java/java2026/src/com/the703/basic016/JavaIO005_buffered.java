...
- new BufferdReader(new InputStreamReader(new FilInputStreamReder(file)))
-1) new FileInputStreamReder(file) 	- byte단위로 읽어오기 (0101...)
-2) new InputStreamReader			- byte를 [문자]스트림으로 텍스트 처리 가능하게 변환 , 인코딩
-3) new BufferdReader				- 속도향상, (a -> a (x), abc -> abc(o))한꺼번에 데이터 처리
