...
<body>
   <div class="container card my-5">
      ...
      1. 출력 EL(EL(Expression Language, 표현식 언어))
      2. 자바코드없이 값 출력, jstl와 같이 사용
      </pre>
      <p><%out.println("Hello"); %></p>
      <p><%= "Hello" %></p>
      <p> ${"hello"} ${1} ${3.14} ${'A'} ${"sally"} </p> 
      <p><a href="jsp20_result.jsp?name=sally" class="btn btn-danger">PARAM - 데이터 넘기기</a> </p>
      
      <% // 서블릿 - 비즈니스로직
      Java001 data = new Java001(10,3);
      request.setAttribute("data", data);       
      %>
      <%=request.getAttribute("data") %><!-- 기존 -->
      <p>${data}  </p><!-- 바뀐 후 -->
      <p>${data.a}</p><!-- 객체명.변수명 - getter() 셋팅되어있어야함-->
      
      <hr/>
      <hr/>
      <hr/>
      
      <h3 class="card-header mt-3">002. jstl</h3>
      <pre  class="alert alert-info">
      ...
      <%@taglib  prefix="c"  uri="http://java.sun.com/jsp/jstl/core" %> <!-- jar 파일 다운로드 안하면 오류 -->
      ...
      <c:set var="numbers" value="1,2,3"/>   <!-- value값을 var에 담음 -->
      <c:forEach var="i" items="${numbers}"> <!-- 향상된포문과 비슷-->
      ${i} <br/>      	      
      </c:forEach>
      ...
      %>
      <c:forEach var="i" items="${numbers2}" varStatus="status"> <!-- status : 인덱스  -->
      ${status.index } - ${i} <br/>
      </c:forEach>
   </div>
