...
<body>
	<div class="container card my-5">
		...
		3. request(요청), response(응답), session(정보저장), out(출력) <!-- 선언x impiortx -->
		</pre>
		...
		<form action="jsp013_1.jsp" method="get" onsubmit="return check()">
			<div class="my-2">
				<label for="query">검색어</label> <input type="text"
					class="form-control" placeholder="검색어를 입력하세요." id="query"
					name="query" />
		...
		= getParameterValues("name") <!-- getParameterValue"s" -->
		</pre>

		<form action="jsp013_2.jsp" method="get"
		...
		<h4>003. response</h4>
		<pre class="alert alert-info">
		1.http요청에 대한 응답 저장하는 객체
		2. jsp에서 다른페이지로 이동
			(1) sendRedirect 		- 브라우저 url을 변경, requesto/response객체가 유지안됨.
			(2) RequestDispatcher 	- 위임(요청전달) 
									  브라우저 url을 변경x, requestx/response객체가 유지 됨. 
		
		</pre>
		<%
		// ver-1 알림창 처리안됨.
		// out.println("<script>alert('안녕?');</script>"); // sendRedirect 처리 시 알림창 동작안됨
		// response.sendRedirect("http://www.naver.com"); // 서버 실행 시 바로 네이버로 넘어감		
		%>
		<%
		// ver-2 jsp013_3.jsp 
		...
		: 나이입력받는 폼
								→  jsp013_4.jsp  처리 (2)       
							        ...
							        19세 이상이라면 -  보여주는 주소표시창줄은   jsp013_4.jsp  
							        			   보여지는 화면은 jsp013_adult.jsp 
         
		</pre>
		<form action="jsp013_4.jsp" method="get" onsubmit="return checkAge()">
			<div class="mb-3 mt-3">
				<label for="userage" class="form-label">나이입력:</label> 
				<input type="text" class="form-control" placeholder="나이를 입력하세요." id="userage" name="userage">
				
			</div>
			<div class="my-3">
				<button type="submit" class="btn btn-primary" id="orderBtn" name="orderBtn">성인여부</button>
			</div>
		</form>
		...
		<h4 class="alert alert-success">005. error</h4>
		<pre>
		error
		1. 4xx : 클라이언트오류
		...
		*해결방안
		1. error 페이지만들기
		2. [src] - [main] - [webapp] - [WEB-INF] - web.xml 에러처리설정	
