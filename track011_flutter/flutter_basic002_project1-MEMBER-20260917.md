...
# 2) dio : Axios 처럼 비동기 http통신 라이브러리 (Intercepter 기능제공)
# 3) flutter_secure_storage : 브라우저의 localStorage/Cookies 대신 모바일 암호화 영역 jwt저장
# 4) image_picker :
...
class App extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      title: 'bora', // 앱의 대표 타이틀
      debugShowCheckedModeBanner: false, // 우측상단 debug 띠 제거
...
//os 플랫폼(Android, ios, window 등) 감지 라이브러리
...
```dart
// React 에서 레이아웃 : <Layout>{children}</Layout>
// ConsumerWidget 상속받으면 React 에서 useSelector 처럼 전역인증
...
// AuthState  - Redux
// _dio.interceptors : api 요청마다 `Bearer ${token}` 을 헤더주입, 401에러 토큰
...
class AuthNotifier extends Notifier<AuthState> {
  ...
  late final Dio _dio; // late (나중에 - 사용하기 직전에 초기화) , final 변경x
  // OS 암호화 저장소 객체 생성 (localStorage 대신 모바일 보안 영역 사용)
  final _storage = const FlutterSecureStorage(); // os 암호화 로컬저장소 생성
  ...
  void _initDio() {
    ...
    _dio.Interceptor.add(InterceptorsWrapper(
      // 매 api 요청마다 SecureStorate 에서 토큰 읽어와서 Authorization 해서 주입
