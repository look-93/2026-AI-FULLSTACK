...
api.interceptors.request.use(
  (config) => {
    if (typeof window !== "undefined") {  //CSR 환경에서만 localStorage에 접근
      const accessToken = localStorage.getItem("accessToken");  //저장된 Access Token 가죠오기
      if (accessToken) {
        config.headers.Authorization = `Bearer ${accessToken}`;  // Authorization 헤더에추가
      }
    }
    return config;  
  },
