...
게시판 수정
*부분수정     > 지난시간 260807
*게시판 리스트 > boot - List<PostResponseDto>
*이미지-캐로셀
*해쉬태그
...
■ 핵심정리
1. JWT VS 세션
- 세션 : 서버 메모리에 사용자 상태를 저장 -> 서버확장 시 부담
        (서버에서 출입명단을 직접들고 있기)
- JWT (JSON WEB TOKEN) : 토큰자치에 인증정보를 포함 -> 
        (사용자가 직접 출입증을 들고 다니기)

2. Access Token vs Refresh Token
- Access Token : 짧은 기간 유효 (출입증) -> api 호출시 사용
- Refresh Token : 긴 기간 (장기 체류증) -> redis 냉장고에 보관 안전보관
...
1) security + jwt + redis
  ...
  - JwtAuthenticationFilter  : 보안게이트
    * 매 요청마다 Authorization 헤더 확인 -> 토큰검증 -> SecurityContext 에 사용자 정보 저장
...
jwt:
  issuer: thejoa703
  secret: this-is-a-very-long-random-secret-key-64chars-minimum-1234567890!@#$%^&*() //만료시간, secret key
