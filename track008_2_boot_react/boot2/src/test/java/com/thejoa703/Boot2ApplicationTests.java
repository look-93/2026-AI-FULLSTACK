...
class Boot2ApplicationTests {
	...
	void testHashtagRepository() {
		...
		tag.getPosts().add(post); //Hashtag: List<Post> posts / Post: List<Hashtag> hashtags
		postRepository.save(post);
	...
	@Test
	@DisplayName("■ CommtentRepository-CRUD")
	void testCommtentRepository() {
		// 댓글 생성
	...
	void testDeptUserMapper() {
		DeptUser dept1 = new DeptUser();
	      ...
	      //Q3. 검증 assertThat
	      assertThat(found.get(0).getDname()).contains("영업");
	}	
}
