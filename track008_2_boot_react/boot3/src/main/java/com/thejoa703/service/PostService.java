...
//		return postRepository.findByDeletedFalse();// 삭제되지 않은값		
//	}

	public List<PostResponseDto> getAllPosts(){		 
		return postRepository.findByDeletedFalse().stream().map(PostResponseDto::from).toList();
	}
	...
	public PostResponseDto updatePost(Long userId ,Long postId, PostRequestDto dto, List<MultipartFile> files) {
		...
		//해쉬태그 (1. 겹치면 안됨 2. #해쉬 #test)
		if(dto.getHashtags() != null && !dto.getHashtags().isEmpty()) {
			post.getHashtags().clear(); // 기존태그 삭제 / 해쉬태그 초기화 *******
