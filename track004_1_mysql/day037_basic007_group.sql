...
-- error
select ban, 
       count(*) -- (ban : 집계안된 일반), 1(count : 집계)
from group_userinfo; 
-- 0	25	10:43:12	select ban, count(*) from group_userinfo LIMIT 0, 1000	
...
select sum(comm) as `급여합계`  -- null 뺴고 집계
from emp;
...
select count(comm) as `추가수당을받는 사원수`
from emp;
-- +------------------------+
-- | 추가수당을받는 사원수       |
-- +------------------------+
...
select sal as `부서10 최대급여 `
  from emp
where deptno = 10
order by sal desc
limit 1;

select MAX(sal) as `부서10 최대급여`
 from emp
where deptno = 10;
...
select sal as `부서10 최소급여 `
  from emp
where deptno = 10
order by sal asc
limit 1;

select MIN(sal) as `부서10 최소급여`
 from emp
where deptno = 10;
...
select hiredate as `부서20 최근입사일`
  from emp
where deptno = 20
order by hiredate desc
limit 1; 

select MAX(hiredate) as `부서20 최근입사일`
  from emp
where deptno = 20;
...
select hiredate as `부서20 제일오래 된 입사일`
  from emp
where deptno = 20
order by hiredate asc
limit 1; 

select MIN(hiredate) as `부서20 제일오래 된 입사일`
  from emp
where deptno = 20;
...
select avg(distinct sal) as `부서30 중복제거 평균급여`
from emp
where deptno = 30;
-- +--------------------------+
-- | 부서30 중복제거 급여평균 |
