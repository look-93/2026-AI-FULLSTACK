...
alter table join_userinfo modify no int not null primary key auto_increment;
desc join_userinfo;
...
insert into join_userinfo value(1, 'first', 11);
insert into join_userinfo value(2, 'second', 22);
insert into join_userinfo value(3, 'third', 33);
insert into join_userinfo value(4, 'fourth', 44);
...
-- |  6 | sixth  | C    |
-- +----+--------+------+
insert into join_userban value(1, 'first', 'A');
insert into join_userban value(2, 'second', 'B');
insert into join_userban value(3, 'third', 'A');
insert into join_userban value(4, 'fourth', 'C');
insert into join_userban value(5, 'fifth', 'B');
insert into join_userban value(6, 'sixth', 'C');
...
-- >>> (1) = join
select a.*, b.ban
from join_userinfo as a, join_userban as b
where a.no = b.no;

-- >>> (2) join  on
select a.*, b.ban
from join_userinfo as a
join join_userban as b on a.no = b.no;

-- >>> (3) join  using
select a.*, b.ban
from join_userinfo as a
join join_userban as b 
using (no);
...
from join_userinfo as a, join_userban as b
where a.no = b.no
  and a.age >= 20 and a.age <= 40;

-- >>> (2) join  on
select a.*, b.ban
from join_userinfo as a
join join_userban as b on b.no = a.no
...
select no, name, ban, age
from join_userinfo  natural join join_userban
where age >= 20 and age <= 40;

-- question3)  outer join
select b.*, a.ban
from join_userban as a
left outer join join_userinfo as b on b.no = a.no;
...
select b.*, a.age
from  join_userinfo as a
right outer join join_userban as b on b.no = a.no;
...
-- 0	96	10:12:51	SELECT EMPNO, ENAME, DEPTNO, DNAME, LOC
--   FROM EMP E, DEPT D
--   WHERE E.DEPTNO = D.DEPTNO
--  LIMIT 0, 1000	Error Code: 1052. Column 'DEPTNO' in field list is ambiguous	0.016 sec
...
-- --------------------------------------------------------
-- --------------------------------------------------------
-- -- 실습 8 select*from SALGRADE; select * from dept; select * from emp;
-- -- emp 테이블을 두번 이용하여
-- -- 별칭을 이용하여  EMP테이블의 사원의 급여가  SALGRADE의 최소급여와 최대급여사이이 있는
...
from emp as a
join emp as b on b.empno = a.mgr
join SALGRADE as c on a.sal between c.losal and c.hisal
order by a.mgr;
...
select  a.empno, 
        ...
        b.empno as `MGR_EMPNO`,
        b.ename as `MGR_ENAME`
from emp as a, emp as b, SALGRADE as c
where b.empno = a.mgr 
...
select  a.empno, 
        ...
        b.ename as `MGR_ENAME`
from emp as a
left outer join emp as b on b.empno = a.mgr;
...
select  a.empno, 
        ...
        -- , a.*, b.*
from emp as a
right outer join emp as b on b.empno = a.mgr
order by a.empno, b.job desc;

-- 기준 테이블이 다 나오되 없는데이터는 null로 표기
...
select a.empno,
       ...
       b.loc
from emp as a
join dept as b on b.deptno = a.deptno
where a.sal <= 3000
order by a.deptno;
...
select a.deptno,
	   b.dname      as `DNAME`,
       avg(a.sal)   as `AVG_SAL`,
       max(a.sal)   as `MAX_SAL`,
       min(a.sal)   as `MIN_SAL`,
       count(a.deptno)     as `CNT`
from emp as a, dept as b
where a.deptno = b.deptno
group by a.deptno, b.dname
order by a.deptno, b.dname;
...
select a.deptno,
       ...
       b.sal
from dept as a
left outer join emp as b on b.deptno = a.deptno
...
select b.deptno,
       ...
       a.sal
from  emp as a
right outer join dept b on b.deptno = a.deptno
...
-- --------------------------------------------------------
-- --------------------------------------------------------
-- -- 실습 20 select*from SALGRADE; select * from dept; select * from emp;
...
select a.deptno,
       ...
       c.hisal,
	   c.grade,
       d.empno as `MGR_EMPNO`,
       d.ename as `MGR_ENAME`
from dept as a
left outer join emp      as b on b.deptno = a.deptno
left outer join SALGRADE as c on b.sal between c.losal and c.hisal
left outer join emp      as d on d.empno = b.mgr
