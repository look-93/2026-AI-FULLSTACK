...
select  a.deptno, 
		...
		a.ename, 
		a.sal
from emp as a natural join dept as b
where a.sal > 2000;
...
select a.deptno, 
	   ...
	   b.sal
from dept as a
left join emp as b on b.deptno = a.deptno
...
select a.deptno, 
	   ...
	   b.sal
from emp as b
right join dept as a on a.deptno = b.deptno
order by a.deptno, b.ename;
...
select *
from sub_userinfo
where age >= (select avg(age) as age from sub_userinfo);
...
select *
from emp
where sal > (select sal from emp where sal = 2975);
...
select *
from emp
where (sal,deptno) = any (select max(sal), deptno from emp group by deptno);
...
select *
from emp
where (sal,deptno) = some (select max(sal), deptno from emp group by deptno);
