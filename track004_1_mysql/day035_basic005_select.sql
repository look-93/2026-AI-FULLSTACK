...
select distinct name from userinfo; -- 중복제거
-- select no, distinct name from userinfo; -- distinct 위치 error
...
select no as `번호`, name as `이름`, age*100/0.9  from userinfo;
select no as `번호`, name as `이름`, age*100/0.9 as `나이값계산` from userinfo;
...
-- select * from userinfo where name ^= 'second'; -- 다르다
...
select * from userinfo where age between 22 and 33;  -- 22 와 33 포함? o
select * from userinfo where age >= 22 and age <=33;

-- ■ SQL  SELECT(8) 일치하는 레코드추출(IN연산자)
--  [ SELECT  [필드명] FROM  [테이블이름] WHERE 컬럼명 IN(값1, 값2, 값3,,,)  ] 
select * from userinfo where age in (22,33);
...
--  a% (a로 시작하는 문자열), %b (b로 끝나는 문자열), %a%(a가 포함되는), _a%(두번째 글자가 a)
-- ename >= 'C' (첫문자 C와 같거나 뒤쪽인 사원) C,D,E,F,,,
select * from userinfo where name = 'a%';     -- x 똑같은애만 가능  
...
select*from userinfo where age not in(22,44); 			-- 22이거나 44살이 아닌(not in) 

select * from userinfo where name like 'f%';			-- 이름이 f로 시작하는
select * from userinfo where name not like 'f%';		-- 이름이 f로 시작하지 않는

select * from userinfo where age is not null; 			-- age 값이 없는애들
select * from userinfo where age is null; 				-- age 값이 있는애들